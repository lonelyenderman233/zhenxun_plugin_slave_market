...
async def _(bot:Bot, event: GroupMessageEvent):
    ...
    if not user:
        ...
        await BagUser.add_gold(at[0],group_id,m)
        ...
        if not m:
            ...
            if i := await UsersInfo.remove_user(user.muser_qq,group_id,at[0]):
                ...
                await BagUser.spend_gold(user_id,group_id,m)
                await BagUser.add_gold(at[0],group_id,m/10)
                a = await BagUser.get_gold(at[0],group_id)
                u = await BagUser.get_gold(user_id,group_id)
...
@work.handle()
async def _(bot:Bot, event: GroupMessageEvent):
    present = datetime.now(pytz.timezone('UTC')).replace(tzinfo=pytz.timezone('UTC'))
    group_id = event.group_id
    user_id = event.user_id
    user, is_create = await UsersInfo.get_or_create(user_qq=user_id, group_id=group_id)
    print(user.checkin_time_last.replace(tzinfo=pytz.timezone('UTC')))
    print(present)
    if user.checkin_time_last.replace(tzinfo=pytz.timezone('UTC')) + timedelta(hours=3) >= present:
